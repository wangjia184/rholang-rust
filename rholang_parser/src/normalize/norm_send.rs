...
impl super::Normalizer {
    ...
    pub fn normalize_send(&mut self, proc : &RawProc,  input: &ProcVisitInputs) -> Result<ProcVisitOutputs, CompiliationError> {
        // normalize the name
        ...
        if name_ == 0 as bnfc::Name {
            return Err(CompiliationError::new_null_pointer("psend_.name_"));
        }
        if send_ == 0 as bnfc::Send {
            return Err(CompiliationError::new_null_pointer("psend_.send_"));
        }
        if listproc_ == 0 as bnfc::ListProc {
            return Err(CompiliationError::new_null_pointer("psend_.listproc_"));
        }
        let n = unsafe { *name_ };
        ...
        let persistent_send = match kind {
            ...
            _ => {
                return Err(CompiliationError::new_unrecognized_data("psend_.send_.kind", kind, proc.line_number, proc.char_number) );
            }
        };
    ...
    pub fn list_proc(&mut self, mut listproc_ : bnfc::ListProc) -> Result< Vec<bnfc::Proc>, CompiliationError>
    {
        ...
        while listproc_ != 0 as bnfc::ListProc
        {
            ...
            if listproc_ == 0 as bnfc::ListProc {
                return Err(CompiliationError::new_null_pointer("psend_.listproc_.proc_"));
            } else {
                list.push(proc_);
            }
